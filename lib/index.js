...
export default async (config) => {
    ...
    if (config.options) {
        config.options = ul.deepMerge(
            config.options,
            config.default_options || {}
        )
    }

    const args = config.uri ? [
        config.uri,
        config.options
    ] : [
        config.db_name,
    ...
    // Set the models
    for (const [n, c] of Object.entries(config.models)) {
        models[camelo(n, true)] = c
        // Handle JSON fields for MySQL, since MySQL doesn't support them natively
        if (shouldHandleJson && c.refreshAttributes) {
            for (const [attrName, attr] of Object.entries(c.rawAttributes)) {
